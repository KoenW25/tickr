...
const FROM = 'Tickr <onboarding@resend.dev>';

function layout(content) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://tickr-five.vercel.app';
  return `
<!DOCTYPE html>
<html>
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);">
        <tr><td style="background:#0f172a;padding:24px 32px;">
          ...
          <span style="color:#f8fafc;font-size:18px;font-weight:600;margin-left:8px;vertical-align:middle;">Tickr</span>
        </td></tr>
        ...
        <tr><td style="padding:16px 32px 24px;border-top:1px solid #e2e8f0;">
          <p style="margin:0;font-size:12px;color:#94a3b8;text-align:center;">
            &copy; ${new Date().getFullYear()} Tickr &mdash; Veilig tickets kopen en verkopen
          </p>
...
export async function sendWelcomeEmail(to) {
  ...
  const html = layout(`
    <h1 style="margin:0 0 8px;font-size:22px;color:#0f172a;">Welkom bij Tickr! 🎉</h1>
    <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.6;">
      Leuk dat je erbij bent! Met Tickr kun je veilig en gemakkelijk tickets kopen en
...
export async function sendBuyerConfirmationEmail(to, eventName, amount) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://tickr-five.vercel.app';
...
export async function sendWaitlistConfirmationEmail(to) {
  ...
  const html = layout(`
    <h1 style="margin:0 0 8px;font-size:22px;color:#0f172a;">Je staat op de Tickr wachtlijst! ✅</h1>
    <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.6;">
      Bedankt voor je aanmelding. We sturen je een e-mail zodra Tickr live gaat.
    </p>
    <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.6;">
      Intussen kun je alvast meer lezen over hoe Tickr werkt en de markt volgen.
    </p>
  ...
  const { data, error } = await resend.emails.send({
    from: FROM,
    to,
    subject: 'Je staat op de Tickr wachtlijst 🎟',
    html,
  });
